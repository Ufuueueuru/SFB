...
function Character(pic) {
	...
	if(this.pic === randomPic) {
		this.random = Math.ceil(Math.random()*11);
	...
	if(true) {
		...
		if(this.pic === groudonPic) {
			this.animation = new Animation(0, 0, 36, [groudon4, groudon5, groudon6, groudon7], [], [groudon0, groudon1, groudon2, groudon3], []);
		}
	}//animations
...
/*
Version History:
0.1.3 Pre and back: Built game physics and fixed bugs
0.1.4 Pre: Added Toad to Smash roster and sound
0.1.5 Pre: Added Professor E. Gadd
0.1.6 Pre: Added Groudon to Smash roster
*0.1.7 Pre: Added mobile support for controls

*Not yet released

*/

var version = "0.1.7";//The version of the game
var pre = "Pre";//If the game is Pre, Alpha, Beta, etc.
var pictures = 85;//85 as of version 0.1.7 Pre

var developerMode = false;

var mobileControls = new MobileControls(false);
...
function AI() {
	...
	this.calculate = function(i) {
		...
		if(player[i].x - player[this.target[i]].x < 30 && player[i].x - player[this.target[i]].x > -30) {//Attempts to get rid of stale mates
			if(random(0,1) > .5) {
				cpuControls[i].right = true;
				cpuControls[i].left = false;
			} else {
...
function Player(x, y, cpu, damage, xVel, yVel, look, dir, attacking, inv, alive, airSpecial, team, character) {
  ...
  this.character = character;//The character object
	...
	this.name = function() {
		...
		if(this.character.pic === groudonPic) {
			return "Groudon";
		}
	}
}
function Arena(name) {
	...
	this.formArena = function() {
		...
		if(this.name === "Tower") {
			...
			this.bounds = [-390, 270];
			this.blocks = [
				new Block(-395, 202, 647, 162),
				new Block(-391, 24, 97, 74),
				new Block(-128, 124, 123, 146),
				new Block(141, 5, 130, 92)
			];
		}
...
function MobileControls(yes) {
	this.active = yes;
	
	this.buttons = [
		new Button(250, 225, 150, "Up"),
		new Button(250, 425, 150, "Down"),
		new Button(145, 325, 150, "Left"),
		new Button(355, 325, 150, "Right"),
		new Button(780, 400, 150, "Attack"),
		new Button(620, 400, 150, "Special"),
		new Button(700, 270, 150, "Shield"),
		new Button(450, 60, 80, "Pause")
	];
	...
	this.controls = function(touchX, touchY) {
		if(this.active) {
			keys[controls[0].pause] = false;
			keys[controls[0].shield] = false;
			keys[controls[0].right] = false;
			keys[controls[0].left] = false;
			keys[controls[0].down] = false;
			keys[controls[0].up] = false;
			keys[controls[0].attack] = false;
			keys[controls[0].special] = false;
			
			for(var i = 0;i < touches.length;i++) {
				for(var u = 0;u < this.buttons.length;u ++) {
					if(this.buttons[7].collide(i)) {
						keys[controls[0].pause] = true;
					}

					if(this.buttons[6].collide(i)) {
						keys[controls[0].shield] = true;
					}

					if(this.buttons[5].collide(i)) {
						keys[controls[0].special] = true;
					}

					if(this.buttons[4].collide(i)) {
						keys[controls[0].attack] = true;
					}

					if(this.buttons[3].collide(i)) {
						keys[controls[0].right] = true;
					}

					if(this.buttons[2].collide(i)) {
						keys[controls[0].left] = true;
					}
					
					if(this.buttons[1].collide(i)) {
						keys[controls[0].down] = true;
					}

					if(this.buttons[0].collide(i)) {
						keys[controls[0].up] = true;
					}
...
function Button(x, y, radius, text = "") {
		this.x = x;
		this.y = y;
		this.radius = radius;
		this.text = text;
		...
		this.collide = function(i) {
			if(dist(touches[i].x, touches[i].y, this.x, this.y) < this.radius/2) {
				return true;
			} else {
				return false;
			}
